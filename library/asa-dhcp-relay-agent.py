...
mainurl= 'https://{}/api/dhcp/relay/agents'.format(m_args['host'])
token = ''

body = {
        "kind": "object#DHCPRelayAgent",
        "setRoute": False,
        "ipv6RelayEnabled": False,
        "ipv4RelayEnabled": m_args['enabled'],
        "interface": m_args['interface'],
        "trusted": False
        }
...
# Probeer aan te melden met een token
    aanmeldpoging()
    with open('%s/tokenfile' %path, 'r') as jar:
        r = requests.post(mainurl, headers={'X-Auth-Token': '%s' % jar.readline()}, json=body, verify=False)
        jar.close()
    #r_json = r.json()
